...
- [x] 日常图片下发与局域网插播任务调度（上游 JPG，服务端统一 dither/BMP 化）
...
- `firmware/photoframe-rs/`：当前设备固件工程（Rust + ESP-IDF）
...
- 当前设备固件路径为 `firmware/photoframe-rs/`
- 构建：`scripts/build-photoframe-rs.sh`
- 远端 Docker 构建：`scripts/build-with-y9000.sh --bootstrap-if-missing`
- 产物：`firmware/photoframe-rs/dist/photoframe-rs-app.bin`、`firmware/photoframe-rs/dist/photoframe-rs-fullchip.bin`
- 现场升级默认使用 `scripts/flash-photoframe-rs.sh` 分段刷写，保留 NVS；`photoframe-rs-fullchip.bin` 只用于空片首刷，误刷会清空设备本地 token / Wi‑Fi / orchestrator 配置
- 当前已接通：NVS 配置、设备身份生成、多 Wi‑Fi 轮询连接、SNTP 校时、orchestrator 配置同步 / 指令拉取、图片下载、BMP/JPEG 渲染、显式 `config/applied` 回报、checkin 上报、按键唤醒判定、AP/STA Portal、深睡进入
- 构建基线文档：`docs/plans/2026-03-07-rust-firmware-rewrite-design.md`
- 实施计划：`docs/plans/2026-03-07-rust-firmware-rewrite.md`
- 里程碑发布说明：`docs/releases/v0.1.0-rust-fw.md`（tag：`v0.1.0-rust-fw`）
- 第一优先级仍是：稳定性、可测试性、省电行为不回退
...
- NAS Web：`http://<NAS_IP>:18081/`（vNext 默认入口；旧控制台位于 `/legacy`）
