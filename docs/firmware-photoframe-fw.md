...
1. **配网（Captive Portal）**
   - 无 Wi-Fi 凭据时会自动进入配网模式。
   - 长按 **BOOT** 上电 3 秒：清 Wi-Fi 并进入配网模式（逃生口）。
   - 长按 **KEY** 上电 3 秒：优先打开 STA 本地配置窗口（若无 Wi-Fi 凭据则仍进入配网模式）。
...
4. **按键语义（KEY/BOOT 区分）**
   ...
   - 为避免 EXT1 ANY_LOW 在深睡阶段因引脚浮空导致误唤醒：深睡前会启用 KEY/BOOT 的 RTC 上拉；唤醒后用短采样窗口确认“按钮曾被按下”，在省电与可用性之间取折中。

5. **多 Wi-Fi 记忆与自动回连**
   - 最多记住 3 组 Wi-Fi 凭据（按最近配置顺序保留）。
   - 唤醒时优先尝试上次成功连接的网络，再依次尝试其他已保存网络。
   - 成功后会自动更新“上次成功网络索引”，下次优先同一网络。
   - 支持通过 orchestrator 下发多 Wi-Fi 凭据（`wifi_profiles`），设备会合并写入并在下次唤醒周期生效。
...
8. **设备心跳上报（编排模式）**
   ...
   - 同时上报 `sta_ip`（设备当前 Wi-Fi IP），便于在路由器/DHCP 列表中快速定位。
...
如果你长按 KEY 打开了 120 秒窗口，请访问设备当前 STA IP（串口日志会打印 URL，例如 `http://192.168.58.120/`）。
...
设备在 STA 模式下会将 DHCP hostname 设置为 `device_id`（例如 `pf-d9369c80`），便于在路由器的 DHCP/客户端列表中直接按名称查找设备。
