...
if [[ -n "${PHOTOFRAME_TEST_POWER_OVERRIDE_JSON:-}" ]]; then
  BUILD_ENV_PREFIX+=("PHOTOFRAME_TEST_POWER_OVERRIDE_JSON=${PHOTOFRAME_TEST_POWER_OVERRIDE_JSON}")
  echo "[info] 将 PHOTOFRAME_TEST_POWER_OVERRIDE_JSON 注入 Docker 构建"
fi
